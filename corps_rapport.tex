...
\begin{itemize}
    ...
    \item \textbf{Les effets de batch :} La plupart des jeux de données sont composés de plusieurs "batchs" (lots expérimentaux). Par exemple, les données de PBMC combinent 16 lots différents, et la rétine de macaque en compte 30. D'autres jeux de données peuvent combiner plusieurs échelles d'effet batch, comme le jeu de données Human Pancreas qui combine différentes technologies de séquençage en plus de différents patients issus de certaines technologies. 
...
Nous avons retenu le jeu de données Human Pancreas (issu de la suite OpenProblems \cite{openproblems_pancreas}) pour les expériences présentées ici. Ce choix permettait de réunir dans un même cadre les deux difficultés au centre du mémoire : un effet de lot marqué et un fort déséquilibre entre types cellulaires. Le jeu de données assemble plusieurs lots issus de technologies de séquençage différentes, notamment \textit{inDrop}, \textit{CEL-Seq}, \textit{CEL-Seq2}, \textit{Fluidigm C1}, \textit{Smart-seq2} et \textit{SMARTer}. Il contient également des données issues du pancréas humain de Baron \cite{Baron2016}, qui constitue un standard de référence largement utilisé pour évaluer les méthodes de clustering en scRNA-seq. Les publications originales de plusieurs méthodes d'apprentissage profond comme scDeepCluster \cite{tian2019clustering}, scNAME \cite{wan_scname_2022}, scMAE \cite{Fang2024scMAE} et scCDCG \cite{xu_sccdcg_2025}, ainsi que des approches dédiées aux populations rares telles que CellSIUS \cite{Neri2019}, scAIDE \cite{scAIDE2020}, DeepScena \cite{Lei2023scHDR}, scCAD \cite{Xu2024scCAD} et le benchmark scCluBench \cite{xu_scclubench_2026}, s'appuient sur ce type de données pancréatiques pour valider leurs performances.

Après filtrage, les expériences sur le jeu de données Human Pancreas portent sur 14\,908 cellules réparties en quatorze types cellulaires. Le tableau~\ref{tab:classes_pancreas} montre que les cellules \textit{alpha} et \textit{beta} représentent à elles seules plus de la moitié du jeu de données, tandis que plusieurs populations d'intérêt biologique, comme les \textit{macrophage}, \textit{mast}, \textit{schwann} ou \textit{t\_cell}, représentent moins de 1\,\% des cellules. Ce jeu de données est donc particulièrement adapté pour tester si un algorithme conserve les petites populations au lieu d'optimiser uniquement les grandes classes.
...
\begin{table}[H]
    ...
    \begin{tabular}{lrrl}
        ...
        \midrule
        \textit{alpha} & 4\,616 & 30,96\,\% & Fréquente \\
        \textit{beta} & 3\,700 & 24,82\,\% & Fréquente \\
        \textit{ductal} & 2\,143 & 14,37\,\% & Fréquente \\
        \textit{acinar} & 1\,671 & 11,21\,\% & Fréquente \\
        \textit{delta} & 1\,007 & 6,75\,\% & Fréquente \\
        \textit{gamma} & 616 & 4,13\,\% & Rare \\
        \textit{activated\_stellate} & 464 & 3,11\,\% & Rare \\
        \textit{endothelial} & 313 & 2,10\,\% & Rare \\
        \textit{quiescent\_stellate} & 193 & 1,29\,\% & Rare \\
        \textit{macrophage} & 79 & 0,53\,\% & Ultra-rare \\
        \textit{mast} & 42 & 0,28\,\% & Ultra-rare \\
        \textit{epsilon} & 32 & 0,21\,\% & Ultra-rare \\
        \textit{schwann} & 25 & 0,17\,\% & Ultra-rare \\
        \textit{t\_cell} & 7 & 0,05\,\% & Ultra-rare \\
        \bottomrule
    \end{tabular}
    \caption{Composition du jeu de données Human Pancreas après filtrage. Les classes rares correspondent aux types représentant moins de 5\,\% des cellules, et les classes ultra-rares à ceux représentant moins de 1\,\%.}
    \label{tab:classes_pancreas}
\end{table}

Différentes expériences ont été menées afin de répondre à une question simple : est-ce qu'un modèle qui appren une représentation sur certaines cellules est ensuite capable de rester performant sur des cellules qu'il n'a jamais vues ? Cette question distingue deux cadres d'évaluation. Dans un cadre transductif, toutes les cellules sont disponibles au moment de l'apprentissage et le modèle construit ses groupes à partir de l'ensemble complet. Ce cadre est utile pour explorer un jeu de données déjà acquis, mais il ne dit pas vraiment ce qui se passerait si l'on ajoutait de nouvelles cellules ensuite. Dans un cadre inductif, au contraire, le modèle est entraîné sur une partie des cellules, puis il doit projeter ou classer de nouvelles cellules sans être réentraîné depuis le début.

Pour rendre cette évaluation possible avec des modèles basés sur des autoencodeurs, nous avons utilisé leur encodeur comme outil de projection. Cette adaptation s'appuie sur une idée déjà présente dans les méthodes de deep clustering comme DEC \cite{xie2016unsupervised} et IDEC \cite{guo2017improved}, où les cellules sont projetées dans un espace latent avant d'être rapprochées de centres de clusters. L'objectif d'appliquer un modèle entraîné à de nouvelles cellules se rapproche quant à lui davantage de la logique de projection de requêtes sur une référence (\textit{query-to-reference mapping}) proposée par scArches \cite{lotfollahi2021query} en génomique sur cellule unique. Concrètement, une fois le modèle entraîné, ses poids sont gelés : cela signifie qu'ils ne sont plus modifiés. Les cellules d'entraînement sont projetées dans l'espace latent, puis les centres des clusters appris sont calculés dans cet espace. Les nouvelles cellules passent ensuite par le même prétraitement et par le même encodeur ; elles sont finalement rattachées au cluster dont le centre est le plus proche, ou à un groupe de référence construit pendant l'entraînement. Cette logique a été appliquée aux modèles pour lesquels une projection de nouvelles cellules était possible. Elle ne peut pas être appliquée de manière automatique à toutes les méthodes, car certaines approches reconstruisent un graphe ou un clustering global à chaque analyse et restent donc principalement transductives. Pour scCDCG, le modèle a été rendu inductif en utilisant uniquement son encodeur entraîné pour projeter individuellement les nouvelles cellules dans l'espace latent, sans reconstruire de graphe. Les cellules sont ensuite assignées au centroïde de cluster le plus proche ou par un vote de majorité parmi leurs $K$-plus-proches-voisins ($K$-NN) d'entraînement.

Dans Human Pancreas, les sous-jeux ne correspondent pas seulement à des donneurs, mais aussi à des technologies expérimentales différentes. Nous avons donc construit les expériences autour de la notion de lot, en conservant deux cadres principaux pour l'analyse des méthodes existantes. Le premier est transductif : l'ensemble des cellules filtrées est analysé simultanément. Le second est inductif : les cellules sont séparées en ensembles d'entraînement, de validation et de test, avec une stratification par lot et par type cellulaire. Pour limiter la domination des grands lots dans le test, nous avons utilisé un split \textit{balanced reinjected} : le test est équilibré entre lots, tandis que les cellules excédentaires sont réinjectées dans l'entraînement. Le principe est résumé dans la figure~\ref{fig:splits_pancreas}.
...
\begin{figure}[H]
    ...
    \begin{tabularx}{0.95\textwidth}{@{}p{0.27\textwidth}X@{}}
        ...
        \midrule
        Données initiales & Neuf lots ou technologies sont disponibles avant filtrage : \textit{celseq}, \textit{celseq2}, \textit{fluidigmc1}, \textit{inDrop1--4}, \textit{smartseq2} et \textit{smarter}. Leurs tailles sont très différentes, ce qui crée un effet de lot visible dès l'analyse exploratoire. \\
        ...
        Split stratifié & Les cellules sont réparties en entraînement, validation et test en conservant autant que possible les proportions de lots et de types cellulaires. \\
        Split \textit{balanced reinjected} & Le test est équilibré entre lots pour éviter qu'un lot majoritaire domine l'évaluation. Les cellules non retenues pour équilibrer le test sont réinjectées dans l'entraînement. \\
        \bottomrule
    \end{tabularx}
    \smallskip
    \hfill\parbox{0.95\textwidth}{\footnotesize\textit{Note : Le mode inductif présenté dans les résultats correspond au split \textit{balanced reinjected}.}}\hfill\null
    \caption{Représentation simplifiée des principaux cadres expérimentaux utilisés sur le jeu de données Human Pancreas.}
    \label{fig:splits_pancreas}
\end{figure}
...
Les résultats obtenus sur Human Pancreas montrent que les métriques globales seules ne suffisent pas à caractériser la qualité biologique du clustering. La figure~\ref{fig:pancreas_inductive_transductive_error_panel} compare les mêmes méthodes dans deux cadres complémentaires : un cadre inductif, évalué sur le test d'un split train/val/test, et un cadre transductif, évalué sur le jeu complet. Le panel présente le taux d'erreur par type cellulaire et par méthode et les métriques globales associées. Le taux d'erreur est défini comme \(1-\text{rappel}\) après appariement hongrois des clusters.

Dans le cadre transductif, plusieurs méthodes obtiennent des scores globaux corrects. Par exemple, scCDCG atteint l'ARI le plus élevé du panel (0,642), et scDeepCluster et scNAME atteignent les meilleurs NMI (environ 0,704 et 0,703). Pourtant, les premières lignes de la heatmap restent très rouges : les cellules \textit{t\_cell}, \textit{schwann}, \textit{epsilon} ou \textit{mast} sont mal retrouvées par la plupart des méthodes. À l'inverse, les grands types cellulaires comme \textit{alpha}, \textit{beta}, \textit{ductal} ou \textit{acinar} dominent plus fortement les scores globaux et peuvent masquer ces erreurs. Ce contraste explique pourquoi la BalancedACC, la RareACC et l'UltraRareACC sont nécessaires pour interpréter les résultats.

Le mode inductif paraît parfois plus favorable pour certaines populations minoritaires, mais cette lecture doit rester prudente. L'évaluation inductive porte sur environ 1\,022 cellules par graine, contre 14\,908 cellules en transductif. Pour une population ultra-rare, le test peut donc ne contenir qu'une ou deux cellules : si elles sont correctement attribuées par hasard, le taux d'erreur devient artificiellement faible, alors que le jeu complet impose de retrouver davantage de cellules et révèle plus clairement les confusions. Cette hypothèse d'un effet de petit effectif est plausible ; elle explique pourquoi certains scores rares semblent meilleurs en inductif sans que cela suffise à prouver une meilleure généralisation. Elle justifie de toujours lire les scores globaux avec les effectifs par type cellulaire, la RareACC, l'UltraRareACC et la stabilité entre graines.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{SCRBenchmark/pancreas_inductive_transductive_error_panel.png}
    \caption{Panel de résultats sur le jeu de données Human Pancreas. A--B : taux d'erreur moyen par type cellulaire et par méthode, en mode inductif sur le test d'un split \textit{balanced reinjected} et en mode transductif sur le jeu complet. C--D : métriques globales associées, moyennées sur cinq graines. Les types cellulaires sont ordonnés par effectif croissant dans le jeu complet. Les scores NMI/ARI peuvent rester corrects alors que l'identification des populations rares dépend fortement du nombre de cellules évaluées.}
    \label{fig:pancreas_inductive_transductive_error_panel}
\end{figure}
