...
\begin{figure}[H]
    ...
    \begin{tabularx}{0.95\textwidth}{@{}p{0.27\textwidth}X@{}}
        ...
        Split inductif & Les modèles sont entraînés sur \textit{human1}--\textit{human3}, puis évalués sur \textit{human4}. Le test contient 1\,303 cellules, dont plusieurs classes ultra-rares représentées par une seule cellule. \\
    ...
    \end{tabularx}
    \smallskip
    \hfill\parbox{0.95\textwidth}{\footnotesize\textit{Note : Le mode inductif présenté dans les résultats correspond au split H123$\rightarrow$H4.}}\hfill\null
...
Les résultats obtenus sur Baron montrent que les métriques globales seules ne suffisent pas à caractériser la qualité biologique du clustering. La figure~\ref{fig:baron_inductive_transductive_error_panel} compare deux cadres complémentaires : un cadre inductif, évalué sur le donneur \textit{human4}, et un cadre transductif, évalué sur le jeu complet. Le panel présente le taux d'erreur par type cellulaire et par méthode ainsi que les métriques globales associées. Le taux d'erreur est défini comme \(1-\text{rappel}\) après appariement hongrois des clusters.
...
\begin{figure}[H]
    ...
    \caption{Panel de résultats sur le jeu de données Baron. A--B : taux d'erreur par type cellulaire et par méthode, en mode inductif sur le test H123$\rightarrow$H4 et en mode transductif sur le jeu complet. C--D : métriques globales associées. Les résultats transductifs sont moyennés sur cinq graines ; le cadre inductif reprend les sorties disponibles pour le split H123$\rightarrow$H4. Les types cellulaires sont ordonnés par effectif croissant dans le jeu complet. Les scores NMI/ARI peuvent rester corrects alors que l'identification des populations rares dépend fortement du nombre de cellules évaluées.}
...
\begin{figure}[H]
    \centering
    \IfFileExists{Images/SCRAW_PIPELINE/scraw_FR.pdf}
      {\includegraphics[width=\textwidth,height=0.72\textheight,keepaspectratio]{Images/SCRAW_PIPELINE/scraw_FR.pdf}}
      {\includesvg[inkscapelatex=false,width=\textwidth,height=0.72\textheight,keepaspectratio]{Images/SCRAW_PIPELINE/scraw_FR}}
...
À la fin de l'entraînement, l'encodeur produit l'embedding final de chaque cellule. Le clustering final est ensuite réalisé avec HDBSCAN dans cet espace latent, avec \texttt{min\_cluster\_size}=8, \texttt{min\_samples}=6 et la stratégie de sélection \texttt{eom}. Les points considérés comme bruit par HDBSCAN ne sont pas réassignés dans cette configuration. Les sorties principales de scRAW sont donc l'espace latent, les labels de clusters prédits, les poids cellulaires finaux, le modèle entraîné, l'historique des pertes et les métriques calculées par le pipeline d'évaluation.

\subsection{Fonctions de perte et variantes}

Dans la configuration \texttt{stable\_default} de \texttt{scRAW\_EXPERIMENTAL}, l'objectif optimisé combine trois termes de perte :
\[
...
\]
Le premier terme reconstruit les profils d'expression, le deuxième contraint localement l'espace latent autour des cellules rares ou isolées, et le troisième réduit l'information de batch visible dans la représentation lorsque plusieurs lots sont présents. Le facteur \(\rho(e)\) augmente progressivement le poids effectif du terme triplet au début de son activation.

\subsubsection*{Reconstruction pondérée}

La perte de reconstruction est le terme principal de scRAW. Dans \texttt{stable\_default}, il s'agit d'une erreur quadratique moyenne (MSE) calculée entre la matrice prétraitée \(X\) et sa reconstruction \(\hat{X}\). Pour une cellule \(i\), on peut l'écrire simplement :
\[
...
La troisième étape a encore réduit l'espace de recherche autour des meilleurs comportements observés, en ne conservant que les familles de paramètres jugées stables : reconstruction MSE, transformation \texttt{log1p}, pseudo-labels Leiden, clustering final hybride, et une plage plus étroite pour les paramètres de pondération, de triplet loss, de DANN et de HDBSCAN. Elle comporte 19 essais, avec arrêt anticipé des candidats trop faibles. Le score utilisé combine le score moyen, l'écart moyen à la meilleure méthode de référence sur chaque dataset, le pire écart observé et le taux de datasets où notre méthode est au moins au niveau de cette référence.

Le meilleur candidat stable présente un score combiné de 0{,}6728 sur 13 datasets évalués. Cette configuration est ensuite devenue la configuration par défaut utilisée dans les expériences présentées ici. Les valeurs exactes sont données en Annexe~\ref{annexe:hyperparametres_scraw}. Les figures d'importance des paramètres sont regroupées en Annexe~\ref{annexe:figures_hyperparametres_scraw}. Elles montrent que les paramètres les plus déterminants changent selon l'échelle de recherche : la stratégie de clustering final et HDBSCAN dominent la recherche Baron, tandis que la recherche multi-datasets fait ressortir le poids minimal des cellules, le nombre d'époques et la correction adversariale. Dans la recherche finale, les paramètres les plus importants sont surtout \texttt{adversarial\_lambda}, \texttt{hdbscan\_min\_cluster\_size}, \texttt{adversarial\_start\_epoch}, \texttt{lr} et \texttt{min\_cell\_weight}. Cela confirme que la configuration finale dépend à la fois de la représentation apprise, de la pondération des cellules rares et de la stratégie de clustering finale.
