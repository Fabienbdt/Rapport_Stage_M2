...
Les résultats obtenus sur le jeu de données Pancreas montrent que les métriques globales seules ne suffisent pas à caractériser la qualité biologique du clustering. La figure~\ref{fig:existant_inductive_transductive_panel} compare les mêmes méthodes dans deux cadres complémentaires : un cadre inductif, évalué sur le test d'un split \textit{balanced reinjected}, et un cadre transductif, évalué sur le jeu complet. Le panel remplace ici la matrice de confusion par un taux d'erreur par type cellulaire, défini comme \(1-\)rappel après appariement hongrois des clusters.

On observe que le mode inductif peut paraître plus favorable que le mode transductif pour plusieurs populations minoritaires, alors que les scores NMI et ARI restent corrects dans les deux cadres. Cette lecture doit toutefois rester prudente. Dans l'expérience inductive, l'évaluation porte sur environ 1\,022 cellules par graine, contre 14\,908 cellules en transductif. Pour une population ultra-rare, le test peut donc ne contenir qu'une ou deux cellules : si elles sont correctement attribuées par hasard, le taux d'erreur devient artificiellement faible, alors que le jeu complet impose de retrouver davantage de cellules et révèle plus clairement les confusions. Cette hypothèse d'un effet de petit effectif est plausible ; elle justifie de toujours lire les scores globaux avec les effectifs par type cellulaire, la RareACC, l'UltraRareACC et la stabilité entre graines.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{SCRBenchmark/pancreas_inductive_transductive_error_panel.png}
    \caption{Panel de résultats sur le jeu de données Pancreas. A--B : taux d'erreur moyen par type cellulaire et par méthode, en mode inductif sur le test d'un split \textit{balanced reinjected} et en mode transductif sur le jeu complet. C--D : métriques globales associées, moyennées sur cinq graines. Les types cellulaires sont ordonnés par effectif croissant dans le jeu complet. Les scores NMI/ARI peuvent rester corrects alors que l'identification des populations rares dépend fortement du nombre de cellules évaluées.}
