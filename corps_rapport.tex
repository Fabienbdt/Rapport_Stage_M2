...
\begin{figure}[htbp]
    ...
    \caption{%
        \textbf{Distribution des performances sur les huit jeux de données communs, par famille de méthodes.}
        Chaque panneau correspond à une métrique (de haut en bas~: ARI, ACC, Rare ACC, Ultra Rare ACC, Correction batch).
        Pour chaque métrique, \textbf{scRAW} (rouge) est affiché en premier, puis les trois meilleures méthodes
        de chaque famille~: \textcolor[HTML]{2563eb}{\textbf{Rare Specific}} (bleu),
        \textcolor[HTML]{ea580c}{\textbf{Méthodes traditionnelles}} (orange) et
        \textcolor[HTML]{16a34a}{\textbf{Correction batch}} (vert).
        Au sein de chaque groupe, les méthodes sont triées par moyenne décroissante (meilleure à gauche).
        Chaque boîte à moustaches représente la distribution des scores sur les huit jeux de données~;
        pour l'Ultra Rare ACC, les jeux sans cellules ultra-rares ne contribuent pas à la distribution.
        Le losange (\(\diamond\)) indique la moyenne.
        La valeur en gras annotée au-dessus de certaines boîtes indique la moyenne des \textbf{deux meilleurs algorithmes toutes familles confondues} pour la métrique considérée.%
    }
...
Cette expérience vise à tester si la loss pondérée utilisée dans \textbf{scRAW} peut être intégrée à d'autres algorithmes de clustering profond. L'objectif est de distinguer l'effet de la pondération rare-aware de l'architecture complète de scRAW. Les méthodes testées sont scMAE, scDeepCluster et DESC.
...
Le passage à un cadre inductif permet d'évaluer si une représentation apprise sur une partie des cellules peut être appliquée à un batch non vu, sans réentraîner entièrement le modèle. Cette situation se rapproche d'un usage réel : on peut imaginer entraîner un modèle sur plusieurs patients déjà caractérisés, conserver ce modèle, puis l'appliquer directement à un nouveau patient inconnu. Dans le cas de Baron Human Pancreas, par exemple, les quatre splits correspondent à une logique \textit{leave-one-patient-out} : le modèle est entraîné sur trois des quatre donneurs \texttt{human1}, \texttt{human2}, \texttt{human3} et \texttt{human4}, puis évalué sur le donneur laissé de côté. Cette logique est particulièrement utile dans un contexte applicatif, où réentraîner un modèle à chaque nouveau patient ou nouveau batch serait coûteux et moins reproductible.

Le protocole général suit cette même idée de séparation par groupe biologique ou technique : patient, donneur, individu macaque, batch ou technologie selon le jeu de données. Pour chaque split, les méthodes sont entraînées uniquement sur les groupes d'entraînement, puis les métriques sont calculées sur le groupe test non vu. La figure~\ref{fig:inductive_metrics_boxplots_trial0017} compare \textbf{scRAW} en mode inductif aux trois méthodes profondes retenues dans cette expérience : scNAME, scMAE et scDeepCluster.
...
Sur ces moyennes par jeu de données, \textbf{scRAW} obtient une moyenne de $0{,}787$ en ARI, $0{,}826$ en ACC, $0{,}752$ en RareACC et $0{,}790$ en UltraRareACC. Le gain est surtout marqué sur l'UltraRareACC, où les autres méthodes restent plus instables et nettement plus basses en moyenne. La RareACC est plus contrastée : certains jeux de données restent difficiles, notamment Kang PBMC, mais scRAW conserve une distribution globalement élevée. Ces résultats suggèrent que la logique rare-aware apprise par scRAW se transfère partiellement au cadre inductif, tout en rappelant que l'évaluation reste sensible au contenu du batch test et aux effectifs parfois très faibles des classes ultra-rares.

\subsection{Interprétation biologique}

Une première analyse biologique post-hoc a été réalisée sur Baron Human Pancreas avec la configuration scRAW . L'objectif n'est pas seulement de vérifier l'accord numérique avec les annotations, mais aussi de regarder si les clusters prédits conservent une cohérence biologique au niveau des gènes marqueurs. Inspiré des protocoles de caractérisation biologique de scCluBench \cite{xu_scclubench_2026} et de scAIDE \cite{scAIDE2020}, ce protocole de validation extrait et compare les signatures d'expression des clusters prédits avec les types cellulaires de référence. Pour chaque type cellulaire de la vérité terrain et pour chaque cluster prédit, les 100 meilleurs gènes différentiellement exprimés (DEG) sont extraits à l'aide d'un test de Wilcoxon bilatéral. Ce calcul statistique est effectué après avoir filtré les gènes exprimés dans moins de 3 cellules, normalisé la somme des comptes par cellule à une cible de 10\,000 (\texttt{target\_sum=1e4}) et appliqué une transformation logarithmique ($\log(x+1)$). Les clusters sont ensuite annotés en les affectant au type cellulaire qui maximise le recouvrement (l'intersection) de leurs gènes marqueurs respectifs, en complément de l'assignation hongroise classique.

Sur Baron, cette annotation par recouvrement atteint une exactitude de $94{,}8\,\%$ et une balanced accuracy de $88{,}9\,\%$, contre $93{,}4\,\%$ et $87{,}0\,\%$ pour le mapping hongrois. Les deux annotations concordent pour $97{,}4\,\%$ des cellules, ce qui indique que la partition apprise par \textbf{scRAW} est globalement compatible avec les marqueurs de référence, tout en autorisant la fragmentation biologique plausible d'un même type cellulaire en plusieurs clusters prédits.
...
\begin{figure}[H]
    ...
    \begin{subfigure}[t]{0.49\textwidth}
        \centering
        \includegraphics[width=\linewidth,height=0.145\textheight,keepaspectratio]{Images/analyse_biologique_scraw_baron_trial0017/tsne_qualitative_panel.png}
        \caption{Projection t-SNE des embeddings.}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.49\textwidth}
        \centering
        \includegraphics[width=\linewidth,height=0.145\textheight,keepaspectratio]{Images/analyse_biologique_scraw_baron_trial0017/marker_overlap_heatmap.png}
        \caption{Recouvrement des marqueurs.}
    \end{subfigure}
    \vspace{-0.4em}
    \begin{subfigure}[t]{0.62\textwidth}
        \centering
        \includegraphics[width=\linewidth,height=0.105\textheight,keepaspectratio]{Images/analyse_biologique_scraw_baron_trial0017/representation_similarity_distributions.png}
        \caption{Similarités cosinus entre embeddings.}
    \end{subfigure}
    \vspace{-0.4em}
