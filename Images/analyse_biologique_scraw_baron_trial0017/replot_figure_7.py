...
With heatmap axes sorted by predicted cluster size (y-axis) and true population size (x-axis).
"""
...
def make_heatmap() -> None:
    ...
    # Load cell counts/sizes to order the axes
    df_tsne = pd.read_csv(TSNE_CSV_PATH)
    
    # 1. y-axis sorting: size of predicted clusters (descending)
    pred_sizes = df_tsne["predicted_label"].astype(str).value_counts()
    df.index = df.index.astype(str)
    sorted_rows = sorted(df.index, key=lambda x: pred_sizes.get(x, 0), reverse=True)
    
    # 2. x-axis sorting: size of true populations (descending)
    true_sizes = df_tsne["true_label"].astype(str).value_counts()
    true_sizes.index = [idx.replace("_", " ") for idx in true_sizes.index]
    sorted_cols = sorted(df.columns, key=lambda x: true_sizes.get(x, 0), reverse=True)
    
    # Place 'quiescent stellate' right next to 'activated stellate'
    if "activated stellate" in sorted_cols and "quiescent stellate" in sorted_cols:
        sorted_cols.remove("quiescent stellate")
        idx_act = sorted_cols.index("activated stellate")
        sorted_cols.insert(idx_act + 1, "quiescent stellate")
    
    # Reorder the DataFrame rows and columns
