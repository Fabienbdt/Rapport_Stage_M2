#!/usr/bin/env python3
"""Rebuild final inductive figures with scRAW values replaced by trial0017.

The public label remains "scRAW"; only the underlying scRAW rows are swapped
from the original default run to the exact trial0017 rerun.
"""
...
COMPARISON_TABLE = (
    ...
    / "15_scraw_trial0017_exact_comparison"
    / "tables"
    / "per_split_values_with_trial0017.csv"
)
TRIAL_SOURCE_RUN = (
    "/data2/fbidet/scRAW_Inductif/results/"
    "inductive_scraw_trial0017_exact_all_datasets_20260507_145430"
)
...
def load_final_per_split() -> pd.DataFrame:
    ...
    base = source[~source["algorithm"].isin(["scraw", "scraw_trial0017"])].copy()
    trial = source[source["algorithm"] == "scraw_trial0017"].copy()
    trial["algorithm"] = "scraw"
    trial["algorithm_label"] = "scRAW"
    trial["source"] = "scraw_trial0017_exact_used_as_scraw_final"
...
def write_tables(per_split: pd.DataFrame, summary: pd.DataFrame) -> None:
    ...
    metadata = {
        "description": "Self-contained final figure bundle with scRAW values replaced by exact trial0017 results.",
        "figures_dir": str(FIGURES_DIR),
        "scraw_label_source": "scRAW points use trial0017 exact rerun, not the older default scRAW run.",
...
def plot_global_without_significance(summary: pd.DataFrame, metric: str, suffix: str) -> None:
    ...
    ax.text(
        0.0,
        1.03,
        "scRAW values are from exact trial0017; each point is one dataset mean.",
...
def plot_mean_by_dataset(summary: pd.DataFrame, metric: str) -> None:
    ...
    ax.text(
        0.0,
        1.02,
        "scRAW bars use exact trial0017 values.",
        transform=ax.transAxes,
        ha="left",
        va="bottom",
        fontsize=10,
...
def plot_dataset_balanced(per_split: pd.DataFrame, dataset_key: str) -> None:
    ...
    ax.text(
        0.0,
        1.02,
        "scRAW uses exact trial0017 values.",
        transform=ax.transAxes,
        ha="left",
        va="bottom",
        fontsize=9,
...
def main() -> int:
    ...
    print("scRAW label now uses exact trial0017 values")
    return 0
