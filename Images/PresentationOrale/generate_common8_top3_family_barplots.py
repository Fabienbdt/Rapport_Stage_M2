...
The figure is rebuilt directly from the stable_generalist consolidated table. scRAW is
the fixed `scRAW (stable_generalist)` row, and the batch-correction metric is set to
...
def load_filtered() -> pd.DataFrame:
    ...
    scraw_stable_generalist = (df["method"] == SCRAW_METHOD) & (
