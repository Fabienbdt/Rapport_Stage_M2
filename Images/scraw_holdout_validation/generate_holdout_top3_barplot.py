#!/usr/bin/env python3
"""Generate holdout validation bar plots for scRAW trial_0017.
...
def load_data() -> pd.DataFrame:
    ...
    df = df[(is_non_scraw | is_scraw_trial0017) & df["method"].isin(allowed_methods)].copy()
...
def draw_barplot(df: pd.DataFrame, selected: dict[str, dict[str, list[str]]]) -> None:
    ...
    fig.suptitle(
        "Validation externe : top 3 par famille et métrique + scRAW trial_0017",
        fontsize=13,
        fontweight="bold",
        y=0.995,
    )
